...
public class ClusterConnectionAdd extends AbstractAddStepHandler {
    ...
    static ClusterConnectionConfiguration createClusterConnectionConfiguration(final OperationContext context, final String name, final ModelNode model) throws OperationFailedException {
        ...
        final long clusterNotificationInterval = CommonAttributes.CLUSTER_NOTIFICATION_INTERVAL.resolveModelAttribute(context, model).asLong();
        final int clusterNotificationAttempts = CommonAttributes.CLUSTER_NOTIFICATION_ATTEMPTS.resolveModelAttribute(context, model).asInt();
