...
public class ConnectionFactoryDefinitionAnnotationParser implements DeploymentUnitProcessor {
    ...
    private BindingConfiguration getBindingConfiguration(final AnnotationInstance connectionFactoryAnnotation) {
        ...
        final AnnotationValue nameValue = connectionFactoryAnnotation.value("name");
        ...
        final AnnotationValue interfaceValue = connectionFactoryAnnotation.value("interfaceName");
        if (interfaceValue == null || interfaceValue.asString().equals(Object.class.getName())) {
            throw MESSAGES.annotationAttributeMissing("@ConnectionFactoryDefinition", "interfaceValue");
        }

        final AnnotationValue resourceAdapterValue = connectionFactoryAnnotation.value("resourceAdapter");
        if (resourceAdapterValue == null || resourceAdapterValue.asString().equals(Object.class.getName())) {
        ...
        final DirectConnectionFactoryInjectionSource directConnectionFactoryInjectionSource =
           new DirectConnectionFactoryInjectionSource(name, interfaceValue.asString(), resourceAdapterValue.asString(), mdr);
    ...
    private TransactionSupport.TransactionSupportLevel asTransactionSupportLocal(final AnnotationInstance annotation, String property) {
        AnnotationValue value = annotation.value(property);
        return value == null ? null : (TransactionSupport.TransactionSupportLevel)value.value();
    }
