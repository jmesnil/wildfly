...
public class MigrateTestCase extends AbstractSubsystemTest {
    ...
    public static final String MESSAGING_SUBSYSTEM_NAME = "messaging";

    public MigrateTestCase() {
        super(MESSAGING_SUBSYSTEM_NAME, new org.jboss.as.messaging.MessagingExtension());
    }

    @Test
    public void testMigrateHA() throws Exception {
        String subsystemXml = readResource("messaging_subsystem_migration_ha.xml");
        ...
        assertTrue(model.get(SUBSYSTEM, MESSAGING_SUBSYSTEM_NAME).isDefined());
        ...
        migrateOp.get(OP_ADDR).add(SUBSYSTEM, MESSAGING_SUBSYSTEM_NAME);
    ...
    private void testMigrateOperation(boolean addLegacyEntries) throws Exception {
        String subsystemXml = readResource("messaging_subsystem_migration.xml");
        newSubsystemAdditionalInitialization additionalInitialization = new newSubsystemAdditionalInitialization();
        KernelServices services = createKernelServicesBuilder(additionalInitialization).setSubsystemXml(subsystemXml).build();
        ...
        assertFalse(model.get(SUBSYSTEM, MESSAGING_SUBSYSTEM_NAME).isDefined());
