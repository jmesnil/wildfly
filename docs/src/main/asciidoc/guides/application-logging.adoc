...
Then, you need to add a dependency to the MicroProfile Config API `org.jboss.logging:jboss-logging` in the `<dependencies>` section:
...
If you run the integration tests with that change, you will only see the logs at the `INFO` level in the standard output:
...
You then need to modify the `wildfly-maven-plugin` configuration to execute this script when WildFly is provisioned:
