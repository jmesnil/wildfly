...
public class MetricCollector {
    ...
    private void registerMetric(MetricMetadata metricMetadata, PathAddress address, String attributeName, MeasurementUnit unit, String description, boolean isCounter, Tag[] tags) {
        ...
        Metadata existingMetadata = MetricRegistries.get(MetricRegistry.Type.VENDOR).getMetadata().get(metricMetadata.metricName);
        if (existingMetadata != null) {
            metadata = existingMetadata;
        } else {
            metadata = Metadata.builder()
                    .withName(metricMetadata.metricName)
                    .withType(isCounter ? MetricType.COUNTER: MetricType.GAUGE)
                    .withUnit(metricUnit(unit))
                    .withDescription(description)
                    .build();
        }
        MetricRegistries.get(MetricRegistry.Type.VENDOR).register(metadata, metric, tags);
    }
